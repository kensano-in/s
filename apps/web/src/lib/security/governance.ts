...
export class ReservedUsernameService {
  private static configPath = path.resolve(process.cwd(), 'src/lib/security/reserved_usernames_config.json');
  private static cachedConfig: ReservedConfig | null = null;
  private static lastLoaded = 0;
  private static lastModified = 0;

  private static loadConfig(): ReservedConfig {
    const now = Date.now();
    // Throttle stats check to once every 5 seconds to preserve disk performance
    if (this.cachedConfig && now - this.lastLoaded < 5000) {
    ...
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        if (!this.cachedConfig || mtime !== this.lastModified) {
          const content = fs.readFileSync(this.configPath, 'utf-8');
          this.cachedConfig = JSON.parse(content) as ReservedConfig;
          this.lastModified = mtime;
          this.lastLoaded = now;
          console.log(`[Security] Loaded reserved usernames config v${this.cachedConfig.version} (hot-reloaded).`);
        }
      }
    } catch (err) {
      console.error('[Security] Failed to load reserved usernames config:', err);
    }

    // Fallback baseline config if file is missing or corrupt
    if (!this.cachedConfig) {
      this.cachedConfig = {
        version: "0.0.0-fallback",
        reserved: ["admin", "administrator", "support", "mod", "billing", "dev", "owner", "root", "security"],
        government: ["government", "gov", "police", "army", "pmo", "president", "primeminister"],
        companies: ["google", "apple", "microsoft", "amazon", "discord", "stripe", "sbi", "sbibank"],
        internal: ["verlyn", "shincore", "official"],
        profanities: ["fuck", "bitch", "nigger", "porn", "chutiya", "gand"],
        regional: {}
      };
      this.lastLoaded = now;
    }
  ...
  public static getReservedWords(): string[] {
    const config = this.loadConfig();
    return [...config.reserved, ...config.government, ...config.companies, ...config.internal];
  }

  public static getProfanities(): string[] {
    const config = this.loadConfig();
    return config.profanities;
  }

  public static getRegionalWords(countryIso: string): string[] {
    const config = this.loadConfig();
    return config.regional[countryIso.toLowerCase()] || [];
  }
}
...
function substringCheck(normalized: string): { blocked: boolean; term?: string } {
  const reservedWords = ReservedUsernameService.getReservedWords();
  for (const keyword of reservedWords) {
...
function detectImpersonationPatterns(normalized: string): boolean {
  const authorityTerms = ['admin', 'support', 'official', 'team', 'staff', 'mod', 'verified', 'help', 'service', 'hq', 'real', 'care'];
  const impersonationPrefixes = ['its', 'iam', 'im', 'weare', 'we', 'this', 'myname', 'the'];
  const brands = ReservedUsernameService.getReservedWords().filter(w => w.length > 3);

  for (const brand of brands) {
    if (normalized.includes(brand)) {
      // 1. Brand + Authority Term combination (e.g. googlesupport, verlynteam)
      for (const term of authorityTerms) {
        if (normalized.includes(term)) {
          return true;
        }
      }
      // 2. Impersonation Prefix + Brand (e.g. itsgoogle, iamgoogle, imshincore)
      for (const prefix of impersonationPrefixes) {
        if (normalized.startsWith(prefix) || normalized.includes('_' + prefix) || normalized.includes('-' + prefix)) {
          return true;
        }
      }
    }
...
function isVariationOfReserved(normalized: string, reservedWords: string[]): boolean {
  const suffixes = ['123', 'team', 'support', 'app', 'bot', 'help', 'service', 'official', 'real', 'staff', 'mod', '01'];
  const prefixes = ['real', 'official', 'its', 'iam', 'support', 'staff', 'team', 'the'];

  for (const word of reservedWords) {
    if (normalized === word) return true;
    
    // Check with trailing numbers
    if (new RegExp(`^${word}\\d+$`).test(normalized)) return true;
    
    // Check with leading/trailing underscores/dots
    if (normalized === `_${word}` || normalized === `${word}_` || normalized === `.${word}` || normalized === `${word}.`) return true;
    
    // Check combinations with common prefixes/suffixes
    for (const prefix of prefixes) {
      if (normalized === `${prefix}${word}`) return true;
    }
    for (const suffix of suffixes) {
      if (normalized === `${word}${suffix}`) return true;
    }
  }
  return false;
}

function detectPatternSpam(normalized: string, raw: string): { blocked: boolean; reason?: string } {
  const cleanRaw = raw.toLowerCase().trim();
  
  // Only numbers
  if (/^\d+$/.test(cleanRaw)) {
    return { blocked: true, reason: 'Username cannot consist of only numbers.' };
  }

  // Repeated letters/numbers (aaaaa, 11111)
  if (/^(.)\1{4,}$/.test(cleanRaw)) {
    return { blocked: true, reason: 'Repeated character spam detected.' };
  }

  // Keyboard sequences
  const sequences = ['abcdef', 'qwerty', 'asdfgh', 'zxcvbn', '123456'];
  for (const seq of sequences) {
    if (cleanRaw.includes(seq) || normalized.includes(seq)) {
      return { blocked: true, reason: 'Sequential keyboard pattern detected.' };
    }
  }

  // Fake verification patterns (e.g. username_verified, username_official)
  if (cleanRaw.endsWith('verified') || cleanRaw.endsWith('official') || cleanRaw.startsWith('official') ||
      normalized.endsWith('verified') || normalized.endsWith('official') || normalized.startsWith('official')) {
    const base = cleanRaw.replace(/verified|official/g, '').replace(/[._-]/g, '');
    if (base.length > 0) {
      return { blocked: true, reason: 'Deceptive verification suffix detected.' };
    }
  }
...
function detectProfanity(normalized: string): { blocked: boolean; word?: string } {
  const profanities = ReservedUsernameService.getProfanities();
...
function fuzzyMatch(normalized: string): { blocked: boolean; matchedTerm?: string; score: number } {
  ...
  const targets = ReservedUsernameService.getReservedWords();
  ...
  return { blocked: maxScore >= 0.78, matchedTerm, score: maxScore };
}
...
export function validateUsernameGovernance(username: string, isDeveloper?: boolean, countryIso?: string): GovernanceResult {
  const raw = username.trim();

  // ── LAYER 6: Unicode / Script Contamination ─────────
  if (detectScriptMixing(raw)) {
    return { valid: false, reason: 'Mixed scripts or invisible characters detected.', layer: 'L6_UNICODE' };
  }

  // ── LAYER 8: Strict Structural Rules ─────────
  // Format rules always apply regardless of developer status
  if (raw.length > 0 && !/^[a-z0-9_.]+$/.test(raw)) {
    return { valid: false, reason: 'Allowed characters: lowercase letters, numbers, dot (.), and underscore (_).', layer: 'L8_FORMAT' };
  }
  if (raw.startsWith('.') || raw.startsWith('_')) {
    return { valid: false, reason: 'Username cannot start with a dot or underscore.', layer: 'L8_FORMAT' };
  }
  if (raw.endsWith('.') || raw.endsWith('_')) {
    return { valid: false, reason: 'Username cannot end with a dot or underscore.', layer: 'L8_FORMAT' };
  }
  if (raw.includes('..') || raw.includes('__') || raw.includes('._') || raw.includes('_.')) {
    return { valid: false, reason: 'Username cannot contain consecutive or adjacent special characters.', layer: 'L8_FORMAT' };
  }
  const isDevHandle = isDeveloper || raw.toLowerCase() === 's';

  // Length: min 5 for regular users (developer handles like 's' permitted)
  if (raw.length < 5 && !isDevHandle) {
    return { valid: false, reason: 'Username must be at least 5 characters.', layer: 'L8_FORMAT' };
  }
  if (raw.length > 20) {
    return { valid: false, reason: 'Username cannot exceed 20 characters.', layer: 'L8_FORMAT' };
  }

  // ── LAYER 7: Pattern Detection (on raw string) ────────
  const pattern = detectPatternSpam('', raw);
  if (pattern.blocked) {
    return { valid: false, reason: pattern.reason, layer: 'L7_PATTERN', riskScore: 85 };
  }

  // ── LAYER 0: Normalize ──────────────────────────────
  const normalized = normalizeUsername(raw);

  if (normalized.length < 5 && !isDevHandle) {
    return { valid: false, reason: 'Username must be at least 5 characters.', layer: 'L0_NORM' };
  }

  // NOTE: There is NO full governance bypass — not even for developers.
  // All blocklist, banned company, profanity, and AI checks apply to everyone.

  // ── LAYER 10: Centralized Config Loaded Lists ──────────
  const reservedWords = ReservedUsernameService.getReservedWords();

  // ── LAYER 1: Reserved Usernames Direct Match ────────
  if (reservedWords.includes(normalized)) {
    return { valid: false, reason: 'This identifier is globally reserved.', layer: 'L1_RESERVED', riskScore: 100 };
  }

  // ── LAYER 1.5: Variation Match ──────────────────────
  if (isVariationOfReserved(normalized, reservedWords)) {
    return { valid: false, reason: 'This identifier is a variation of a reserved name.', layer: 'L1.5_VARIATION', riskScore: 95 };
  }

  // ── LAYER 2_ROUTING: Reserved system routes ─────────
  const reservedRoutes = new Set([
    'feed', 'messages', 'communities', 'explore', 'trending', 'settings', 'profile', 
    'join', 'login', 'logout', 'signup', 'notifications', 'search', 'home', 'status', 
    'about', 'contact', 'terms', 'privacy', 'admin', 'dashboard', 'moderation', 'arcade', 
    'badges', 'index', 'default', 'user', 'users'
  ]);
  if (reservedRoutes.has(normalized) || reservedRoutes.has(raw.toLowerCase())) {
    return { valid: false, reason: 'This username is a reserved system route.', layer: 'L2_ROUTING', riskScore: 100 };
  }

  // ── LAYER 2_REGIONAL: Regional Reserved Words ────────
  if (countryIso) {
    const regionalWords = ReservedUsernameService.getRegionalWords(countryIso);
    if (regionalWords.includes(normalized) || isVariationOfReserved(normalized, regionalWords)) {
      return { valid: false, reason: 'This identifier is reserved in your region.', layer: 'L2_REGIONAL', riskScore: 100 };
    }
  }

  // ── LAYER 5: Profanity Check ────────────────────────
  const prof = detectProfanity(normalized);
  if (prof.blocked) {
    return { valid: false, reason: `Username contains restricted term: "${prof.word}".`, layer: 'L5_PROFANITY', riskScore: 90 };
  }
  ...
  // ── LAYER 3.5: Impersonation Patterns ────────────────
  if (detectImpersonationPatterns(normalized)) {
    return { valid: false, reason: 'Deceptive brand or authority impersonation pattern detected.', layer: 'L3.5_IMPERSONATION', riskScore: 95 };
  }

  // ── LAYER 9: Fuzzy Similarity ───────────────────────
  const fuzzy = fuzzyMatch(normalized);
  if (fuzzy.blocked) {
    return { valid: false, reason: `Identifier too similar to "${fuzzy.matchedTerm}" (score: ${Math.round(fuzzy.score * 100)}%).`, layer: 'L9_FUZZY', riskScore: Math.round(fuzzy.score * 100) };
  }
