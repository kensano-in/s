...
export function isDisposableEmail(email: string): boolean {
  ...
  // 2. Check for common generated patterns
  // e.g. purely numeric usernames or domains with excessive numbers/random chars
  const namePart = email.split('@')[0];
  
  // Match completely numeric or high-entropy domains (like x1y2z3.com)
  if (/^[0-9]+$/.test(domain.split('.')[0])) return true;
  if ((domain.match(/[0-9]/g) || []).length > 5) return true;
  
  // Pattern matching for typical temp email formats (e.g. random string + generic word)
  if (domain.includes('temp') || domain.includes('throwaway') || domain.includes('trash')) return true;
