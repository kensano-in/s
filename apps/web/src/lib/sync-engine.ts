...
import { submitProfileUpdateDB, type ProfileSyncPayload } from '@/app/(main)/profile/actionsCore';
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (msg.includes('Unauthorized') || msg.includes('JWT') || msg.includes('401')) {
        throw err; // Fail instantly, no point backoff retrying when unauthorized
      }
...
// ProfileSyncPayload is imported from actionsCore.ts
...
export function dispatchProfileSync(
  ...
  syncProfileToSupabase(userId, updates, setSyncStatus).catch((err) => {
    const msg = err?.message || '';
    if (msg.includes('Unauthorized') || msg.includes('JWT') || msg.includes('401')) {
      console.warn('[SyncEngine] Profile sync bypassed: User is unauthorized/logged out.');
    } else {
