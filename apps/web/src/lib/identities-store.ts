  ...
  persist(
    (set, get) => ({
      ...
      addIdentity: (identity) => {
        ...
        if (existingIndex > -1) {
          const existing = state.identities[existingIndex];
          const lastActiveDiff = Date.now() - new Date(existing.lastActive || 0).getTime();
          if (
            existing.username === identity.username &&
            existing.displayName === identity.displayName &&
            existing.avatarUrl === identity.avatarUrl &&
            lastActiveDiff < 300000 // 5 minutes threshold
          ) {
            return true;
          }
        }
      ...
      setEncryptedSession: (id, encrypted) => set((state) => {
        const existing = state.identities.find(i => i.id === id);
        if (existing && existing.encryptedSession === encrypted) return state;
        return {
