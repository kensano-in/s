...
export default function NotifPanel() {
  const isNotifPanelOpen = useAppStore(s => s.isNotifPanelOpen);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload: any) => {
          // Fetch full notification with actor join to prevent missing actor details glitch
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed right-0 top-0 h-full w-full sm:w-[380px] z-50 flex flex-col transition-transform duration-300 ease-out border-l${!isNotifPanelOpen ? ' pointer-events-none' : ''}`}
        aria-hidden={!isNotifPanelOpen}
        style={{
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{ borderColor: 'var(--border)', height: 'auto' }}>
          ...
          <div className="flex items-center gap-2">
            <button type="button" className="btn-ghost text-xs flex items-center gap-1" onClick={handleMarkAllRead}>
              <CheckCheck size={14} />
              Mark all read
            </button>
            <button type="button" className="icon-btn" onClick={() => setNotifPanelOpen(false)}>
              <X size={18} />
            </button>
          </div>
        </div>
        ...
        {/* Tabs */}
        <div className="flex border-b px-3 flex-shrink-0" style={{ borderColor: 'var(--border)', height: 'auto' }}>
        ...
        {/* List */}
        <div className="flex-1 overflow-y-auto py-2" style={{ height: 'auto' }}>
          {loading ? (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${n.is_read ? 'transparent' : (TYPE_COLORS[notifType] || 'var(--v-violet)')}`,
                  }}
                >
                  ...
                  <div className="flex-shrink-0 relative">
                    ...
                    <img
                      src={getAvatarUrl(n.actor?.username || 'system', n.actor?.avatar_url)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>
                        {formatTimeSafe(n.created_at)}
                      </span>
                      <span
                        className="text-[10px] font-semibold uppercase tracking-wide"
                        style={{ color: TYPE_COLORS[notifType] || 'var(--text-tertiary)' }}
                      >
                        {notifType}
                      </span>
                    </div>
