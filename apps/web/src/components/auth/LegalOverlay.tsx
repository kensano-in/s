...
const SUMMARY_CONTENT = {
  contact: {
    ...
    summary: "Verlyn uses your contact information to secure your account, handle account recovery, and configure multi-factor authentication.",
    bullets: ['Encrypted storage', 'Real-time threat detection', 'Multi-device sync']
  },
  upload: {
    title: 'Find Friends',
    icon: <Users className="text-purple-500" size={24} />,
    summary: "Find and connect with people you know through secure contact list matching.",
    bullets: ['One-way hashing', 'Opt-out anytime', 'Teammate suggestions']
  },
  terms: {
    title: 'Terms & Privacy',
    icon: <Info className="text-emerald-500" size={24} />,
    summary: "Join a community governed by strict data protection standards.",
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white tracking-tight">
                    {fullView ? 'Verlyn Legal Terms' : summary.title}
                  </h2>
                  ...
                  >
                    ...
                    <footer className="pt-12 border-t border-white/5">
                       <p className="text-[10px] text-neutral-700 font-bold uppercase tracking-widest leading-loose">
                        Verlyn © 2026. All Rights Reserved.
                       </p>
                    </footer>
                  </motion.div>
                )}
            ...
            <div className="p-4 bg-white/[0.01] border-t border-white/5 flex flex-col sm:flex-row gap-3">
              <button type="button"
                ...
                className="flex-1 h-12 border border-white/10 text-neutral-400 hover:text-white hover:bg-white/5 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2 group"
              >
                {fullView ? (
                  <>Summary View <X size={14} className="group-hover:rotate-90 transition-transform" /></>
                ) : (
                  <>View Full Terms <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" /></>
                )}
              </button>
              <button type="button"
                onClick={() => {
