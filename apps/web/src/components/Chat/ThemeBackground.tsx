...
import { PRESET_THEMES } from "./CustomThemeSelector";
...
/**
 * Swaying Van Gogh sky logic — stable positions via useMemo
 */
function SkySwirls() {
...
export default function ThemeBackground({ themeId }: ThemeBackgroundProps) {
  const themeStyle = useMemo(() => {
    ...
    const match = allThemes.find((t) => t.id === themeId);
    return match?.style || {};
  }, [themeId]);

  const isSvg = typeof themeStyle === "object" && themeStyle !== null && "backgroundImage" in themeStyle && typeof (themeStyle as any).backgroundImage === "string" && (themeStyle as any).backgroundImage.includes("data:image/svg+xml");
      ...
      <AnimatePresence initial={false}>
        {themeId && (
          <motion.div 
            ...
            style={{ 
              ...themeStyle,
              backgroundSize: isSvg ? "100% 100%" : "cover",
              backgroundRepeat: "no-repeat",
              backgroundPosition: "center"
            }}
            className="absolute inset-0 z-0 overflow-hidden pointer-events-none"
          >
            {/* Subtle vignette — only softens edges, doesn't black out content */}
