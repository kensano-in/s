...
const svgBg = (svg: string) => {
  ...
  if (match) {
    const w = match[1];
    const h = match[2];
    processed = svg.replace(match[0], `viewBox='0 0 ${w} ${h}' width='100%' height='100%'`);
  } else {
    const matchDouble = svg.match(/width="(\d+)"\s+height="(\d+)"/);
    if (matchDouble) {
      const w = matchDouble[1];
      const h = matchDouble[2];
      processed = svg.replace(matchDouble[0], `viewBox="0 0 ${w} ${h}" width="100%" height="100%"`);
    }
  }
  return `url("data:image/svg+xml;utf8,${encodeURIComponent(processed)}")`;
};
...
export function CustomThemeSelector({ selectedTheme = 'midnight', blur = 0, updatedAt, onApply }: CustomThemeSelectorProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      try {
        ...
        formData.append("folder", "chat-themes");
        ...
        console.warn("R2 upload failed for custom theme, trying Supabase storage:", apiErr);
      ...
      onApply({ theme_id: publicUrl, theme_blur: blur });
    } catch (err) {
  ...
  const fullThemeId = selectedTheme || 'midnight';
  const parts = fullThemeId.split('|');
  const actualThemeId = parts[0];
  const lastChangedBy = parts[2] || 'System';

  const allThemes = [...PRESET_THEMES, ...CINEMATIC_THEMES, ...AESTHETIC_THEMES];
  const matchedTheme = allThemes.find(t => t.id === actualThemeId);
  const activeThemeName = matchedTheme ? matchedTheme.name : (actualThemeId.startsWith('http') ? 'Custom Photo' : 'Midnight');
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      {/* Active Theme Info / History */}
      <div className="px-5 py-4 bg-white/[0.01] border-b border-white/5 flex flex-col gap-2 shrink-0">
        <div className="flex items-center gap-1.5 text-white/30">
          <CheckCircle2 size={13} className="text-indigo-400" />
          <h4 className="text-[10px] font-bold uppercase tracking-wider">Active Theme Settings</h4>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-1">
          <div className="p-2.5 bg-white/[0.02] border border-white/5 rounded-xl flex flex-col">
            <span className="text-[10px] text-white/40 mb-0.5">Current Theme</span>
            <span className="text-xs font-semibold text-white/90 capitalize truncate">{activeThemeName}</span>
          </div>
          <div className="p-2.5 bg-white/[0.02] border border-white/5 rounded-xl flex flex-col">
            <span className="text-[10px] text-white/40 mb-0.5">Last Changed By</span>
            <span className="text-xs font-semibold text-white/90 truncate">{lastChangedBy}</span>
          </div>
          {lastUpdatedTime && (
            <div className="col-span-2 px-3 py-2 bg-white/[0.01] border border-white/5 rounded-lg flex justify-between items-center text-[10px]">
              <span className="text-white/40">Last Updated</span>
              <span className="text-white/60 font-medium">{lastUpdatedTime}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-24">
        ...
        <div className="grid grid-cols-3 gap-3">
          {PRESET_THEMES.map(t => {
            const isSelected = (selectedTheme?.split('|')[0] || 'midnight') === t.id;
            return (
              <button type="button"
                key={t.id}
