  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("R2/API upload failed, trying direct client-side Supabase upload fallback:", err.message || err);
        try {
          ...
          const timeoutPromise = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error("Supabase Storage upload timed out")), 8000)
          );
          ...
          clearInterval(fakeProgress);
          console.warn("Direct Supabase upload also failed/timed out, falling back to local object URL:", storageErr.message || storageErr);
