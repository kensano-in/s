...
export default function GhostBubble({
  ...
  useEffect(() => {
    if (type === "voice" && mediaUrl && !shattered && !isViewed) {
  ...
  }, [type, mediaUrl, shattered, isViewed]);

  useEffect(() => {
    if (type === "voice" && audioDuration !== null) {
      setCountdown(audioDuration + 2); // 2-second buffer
    }
  }, [type, audioDuration]);

  useEffect(() => {
    if (!revealed) return;
    // Start countdown then shatter
      ...
      <AnimatePresence mode="wait">
        {!revealed ? (
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
              ...
              <span className="text-[10px] text-white/30">
                {type === "image" ? "📷 Photo" : type === "text" ? "💬 Secret Message" : type === "voice" ? "🎤 Voice Note" : "📎 File"} · View once
              </span>
            </div>
          ...
          >
            ...
            {type === "text" && (
              <div 
                className="p-4 rounded-2xl text-[14px] text-white/90 break-words"
                style={{
                  background: isMine ? "rgba(98,0,238,0.2)" : "rgba(255,255,255,0.05)",
            ...
            {type === "voice" && mediaUrl && (
              <div 
                className="p-3 rounded-2xl text-[14px] text-white/90"
                style={{
                  background: isMine ? "rgba(98,0,238,0.2)" : "rgba(255,255,255,0.05)",
                  border: "1px solid rgba(255,255,255,0.1)",
                  minWidth: "220px",
              ...
              >
                <VoicePlayer src={mediaUrl} isMine={isMine} duration={audioDuration || 0} />
              </div>
            )}
            ...
            <motion.div
              ...
              initial={{ width: "100%" }}
              animate={{ width: "0%" }}
              transition={{ duration: type === "voice" && audioDuration !== null ? audioDuration + 2 : 5, ease: "linear" }}
            />
          </motion.div>
        )}
      </AnimatePresence>
