...
export async function generateMetadata({ params }: { params: Promise<{ code: string }> }) {
  ...
  return {
    ...
    description: "You've been invited to a group on Verlyn.",
  };
}
  ...
  if (!success || !group) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-white/50 mb-8 max-w-sm">This invite link is invalid or the group has been deleted.</p>
        <Link href="/" className="px-8 py-3 bg-white/10 hover:bg-white/20 text-white font-bold rounded-2xl transition-all">Return to Home</Link>
      </div>
    );
  }
