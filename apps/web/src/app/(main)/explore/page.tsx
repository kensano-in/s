...
import {
  ...
  Loader2, X, ChevronRight, ImageIcon, MessageCircle, Heart, Flame, Zap
} from 'lucide-react';
import { useState, useEffect, useCallback, useMemo, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { createClient } from '@/lib/supabase/client';
import { getTrendingHashtags, getDiscoveryPosts, getDiscoveryMediaPosts, getSuggestedPeople, getRisingCreators, searchAll } from './actions';
...
function Avatar({ src, username, size = 36 }: { src?: string; username?: string; size?: number }) {
  const fallback = getAvatarUrl(username || 'default');
  return (
    <img
      src={src || fallback}
      alt={username || 'user'}
...
function ExploreContent() {
  useEffect(() => {
    console.log("[FORENSICS] ExplorePage MOUNTED");
    return () => {
      console.log("[FORENSICS] ExplorePage UNMOUNTED");
    };
  }, []);
  ...
  const [showDropdown, setShowDropdown] = useState(!!defaultQuery);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [trendingData, postsData, peopleData, risingData] = await Promise.all([
        getTrendingHashtags(),
        getDiscoveryPosts(),
        getSuggestedPeople(),
        getRisingCreators(),
      ...
      setLoading(false);
      // Lazy-load media posts after main content
      getDiscoveryMediaPosts().then(data => {
  ...
  // ── Live search with debounce + AbortController to prevent race conditions ──
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const thisController = searchAbortRef.current;
              ...
              recentSearches.length > 0 ? (
                <div>
                  <div className="flex items-center justify-between px-4 pt-3 pb-1">
                    ...
                    <button type="button" onClick={clearRecent} className="text-[10px] text-neutral-600 hover:text-neutral-400 transition-colors">
                      Clear
                    </button>
                  </div>
            ...
            {TABS.map(tab => {
              ...
              return (
                <button type="button"
                  ...
                  className={clsx(
                    'flex-shrink-0 flex items-center gap-1.5 text-xs font-semibold px-4 h-8 rounded-full transition-all duration-200',
                    activeTab === tab.id
                      ? 'bg-white text-black shadow-lg'
                      : 'bg-[#141414] text-neutral-500 border border-[#222] hover:border-[#333] hover:text-white'
                  )}
                >
                  <Icon size={11} />
                  {tab.label}
              ...
              <section>
                ...
                <div className="rounded-xl border border-[#1f1f1f] overflow-hidden divide-y divide-[#1a1a1a]">
                  {loading
                    ...
                    ? (
                      <div className="px-4 py-8 text-center">
                        <p className="text-sm text-neutral-600">Nothing trending right now</p>
                      </div>
                    )
                    : trending.map((t, i) => (
                      <div
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {rising.length > 0 && (
                    <span className="ml-auto flex items-center gap-1 text-[10px] text-emerald-500 font-semibold">
                      <Zap size={10} /> Rising Creators
                    </span>
                  )}
                </div>

                {/* Rising creators strip */}
                {rising.length > 0 && !loading && (
                  <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-3 mb-3">
                    {rising.map((u: any) => (
                      <Link
                        key={u.id}
                        href={`/profile/${u.username}`}
                        className="flex-shrink-0 flex flex-col items-center gap-1.5 p-3 bg-[#111] border border-[#1f1f1f] rounded-xl hover:border-[#2a2a2a] transition-colors w-[76px]"
                      >
                        <img
                          ...
                          decoding="async"
                        />
                        <p className="text-[10px] text-white font-semibold truncate w-full text-center">@{u.username}</p>
                        <p className="text-[9px] text-neutral-600">{u.recentPosts} posts</p>
                      </Link>
                    ))}
                  </div>
                )}
                  ...
                  : people.map((u: any) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-white truncate">{u.display_name || u.username}</p>
                        <p className="text-xs text-neutral-500 truncate">@{u.username}</p>
                        {u.bio && parseBio(u.bio).visibleBio && (
                          <p className="text-xs text-neutral-600 truncate mt-0.5">
                            {parseBio(u.bio).visibleBio}
                          </p>
                        )}
                      </div>
          ...
          {activeTab === 'media' && (
            <div className="pt-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                {mediaPosts.length > 0 && <span className="text-[10px] text-neutral-600 ml-auto">{mediaPosts.length} posts</span>}
              </div>
          ...
          : communities.map((c: any) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-neutral-500">{fmt(c.member_count || 0)} members</p>
                {c.description && (
                  <p className="text-xs text-neutral-600 truncate mt-0.5">{c.description.split('||')[0].trim()}</p>
                )}
              </div>
              <ChevronRight size={14} className="text-neutral-700 flex-shrink-0" />
            </Link>
...
export default function ExplorePage() {
  return (
    <Suspense fallback={<div className="p-10 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-white/50" /></div>}>
      <ExploreContent />
    </Suspense>
  );
}
