...
import { Camera, Settings, Grid3x3, FileText, Bookmark, Award } from 'lucide-react';
  ...
  return (
    <div className="space-y-0 animate-fade-in">
      ...
      >
        ...
        <button
          className="absolute bottom-3 right-3 btn-glass text-xs px-3 py-1.5"
          id="edit-banner-btn"
        >
          <Camera size={12} />
          Edit
      ...
      <div className="px-1 pb-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-sm mb-2" style={{ color: 'var(--text-tertiary)' }}>@{CURRENT_USER.username}</div>
            {CURRENT_USER.bio && (
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button className="btn-glass text-sm" id="edit-profile-btn">
              <Settings size={14} />
              Edit Profile
            </button>
          </div>
        ...
        <div className="flex items-center gap-4 mt-4 flex-wrap">
          <div className="karma-badge">
            ⚡ {kFmt(CURRENT_USER.karmaScore)} Karma
          </div>
          <div className="flex items-center gap-4 text-sm">
            <div className="text-center cursor-pointer hover:opacity-80 transition-opacity">
              <div className="font-bold" style={{ color: 'var(--text-primary)' }}>{kFmt(CURRENT_USER.followerCount)}</div>
              <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Followers</div>
            </div>
            <div className="text-center cursor-pointer hover:opacity-80 transition-opacity">
              <div className="font-bold" style={{ color: 'var(--text-primary)' }}>{kFmt(CURRENT_USER.followingCount)}</div>
              <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Following</div>
            </div>
            <div className="text-center">
              <div className="font-bold" style={{ color: 'var(--text-primary)' }}>{MOCK_POSTS.length}</div>
      ...
      <div className="pt-4 space-y-4">
        {tab === 'posts' && MOCK_POSTS.map((p) => <PostCard key={p.id} post={p} />)}
        {tab === 'saved' && MOCK_POSTS.filter((p) => p.isSaved).map((p) => <PostCard key={p.id} post={p} />)}
        {tab === 'awards' && (
