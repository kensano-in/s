...
export default function TrendingPage() {
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass-card p-4">
        <h2 className="font-bold text-sm mb-3" style={{ color: 'var(--text-primary)' }}>🔥 Fastest Growing Communities</h2>
        <div className="space-y-2">
          {MOCK_COMMUNITIES.filter((c) => c.boostLevel >= 2).map((c) => (
            <div key={c.id} className="flex items-center gap-3 p-2 rounded-xl cursor-pointer hover:opacity-90 transition-opacity">
              <span className="text-2xl">{c.iconUrl}</span>
              <div className="flex-1">
                <div className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{c.displayName}</div>
                <div className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{(c.memberCount / 1000).toFixed(0)}K members</div>
              </div>
              <button className="btn-primary text-xs px-3 py-1.5 shine">Join</button>
            </div>
          ))}
        </div>
      </div>
